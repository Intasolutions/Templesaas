            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 px-4 md:px-0">
                <div className="space-y-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 tracking-tighter uppercase leading-none">Fiscal Ledger</h1>
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-1">
                                        <span className={`text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${CATEGORY_MAP[txn.category]?.bg} ${CATEGORY_MAP[txn.category]?.color}`}>
                                            {CATEGORY_MAP[txn.category]?.label || 'General'}
                                        </span>
                                        <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">{txn.date}</span>
                                    </div>
                                ...
                                <div className="text-right">
                                    <p className={`text-xl font-bold tracking-tighter ${txn.txn_type === 'income' ? 'text-emerald-500' : 'text-slate-900'}`}>
                                    ...
                                    <div className="flex items-center justify-end gap-2 mt-1">
                                         ...
                                         <span className="text-[8px] font-bold text-slate-300 uppercase tracking-[0.2em]">{txn.txn_type} Confirmed</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <div className="space-y-10">
                            ...
                            {summary?.income_by_category?.map(cat => (
                                <div key={cat.category} className="space-y-2">
                                    <div className="flex justify-between items-end">
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">{CATEGORY_MAP[cat.category]?.label || cat.category}</span>
                                        <span className="text-xs font-bold text-white">₹{Number(cat.total).toLocaleString()}</span>
                                    </div>
                    ...
                    <div className="p-8 border border-slate-100 rounded-[2.5rem] bg-white space-y-6 group hover:border-primary/20 transition-all">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Audit Status</p>
                             <p className="text-sm font-bold text-slate-900 uppercase">Registry Verified</p>
                          </div>
                       </div>
                    </div>
                </div>
                    ...
                    <div className="fixed inset-0 z-[500] flex items-center justify-center p-6">
                        ...
                        >
                            <div className="flex justify-between items-start mb-10">
                                <div>
                                    <h2 className="text-2xl font-bold text-slate-900 tracking-tighter uppercase leading-none">Record Transaction</h2>
                            ...
                            <form onSubmit={handleAddTxn} className="space-y-8">
                                <div className="grid grid-cols-2 gap-4">
                                    <button 
                                        ...
                                        className={`h-14 rounded-2xl font-bold text-[10px] uppercase tracking-widest border transition-all ${form.txn_type === 'income' ? 'bg-emerald-50 border-emerald-500 text-emerald-500' : 'bg-white border-slate-100 text-slate-400'}`}
                                    >
                                        Inward (Income)
                                    </button>
                                    <button 
                                        ...
                                        className={`h-14 rounded-2xl font-bold text-[10px] uppercase tracking-widest border transition-all ${form.txn_type === 'expense' ? 'bg-rose-50 border-rose-500 text-rose-500' : 'bg-white border-slate-100 text-slate-400'}`}
                                    >
                                        Outward (Expense)
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em] ml-1">Spatial Category</label>
                                        <select 
                                ...
                                <div className="pt-6">
                                    <button 
                                        disabled={submitting} 
                                        type="submit" 
                                        className="w-full h-16 bg-slate-900 text-white rounded-2xl font-bold text-[11px] uppercase tracking-[0.3em] shadow-2xl shadow-slate-900/40 hover:bg-slate-800 disabled:opacity-50 active:scale-95 transition-all flex items-center justify-center gap-3"
                                    >
...
function MetricCard({ title, value, sub, icon, color, trend }) {
    return (
        <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden group hover:border-primary/20 hover:shadow-2xl hover:shadow-slate-200/50 transition-all">
            ...
            <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em] mb-2">{title}</h3>
            <p className={`text-4xl font-bold tracking-tighter ${color} flex items-baseline gap-2`}>
                ₹{Number(value).toLocaleString()}
                {trend && <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">{value > 0 ? '+ Active' : 'Neutral'}</span>}
            </p>
