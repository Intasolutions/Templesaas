...
import { Zap, Check, ArrowRight, Shield, Globe, Layers, MapPin, Search, Sparkles, Printer, Smartphone } from 'lucide-react';
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                        ...
                        >
                            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white border border-primary/10 shadow-sm mb-10">
                                ...
                                <span className="text-[11px] font-bold uppercase tracking-widest text-wood/80">Core Module</span>
                            </div>
                            <h1 className="text-5xl md:text-[84px] font-black leading-[0.95] tracking-tight text-wood mb-8 uppercase">
                                Pooja <br />
                                <span className="text-primary font-serif italic">Booking.</span>
                            </h1>
                            <p className="text-lg md:text-xl text-wood/70 font-medium leading-relaxed mb-12 max-w-lg">
                                The easiest way to manage pooja bookings at your temple counter. Fast, reliable, and perfectly clear.
                            </p>
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                >
                                    Get a Free Demo <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                                </button>
                            </div>
                        </motion.div>
                            ...
                            <div className="rounded-[2.5rem] border-8 border-white shadow-2xl overflow-hidden bg-wood/5 aspect-square relative">
                                ...
                                <div className="absolute top-8 right-8 bg-white p-6 rounded-2xl shadow-xl border border-primary/10 hidden md:block">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-[10px] uppercase font-bold text-wood/40 tracking-wider">Printer Status</p>
                                            <p className="text-base font-black text-wood tracking-tight">Ready to Print</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <section className="py-32 bg-white">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="text-center mb-20">
                        <h2 className="text-3xl font-black text-wood mb-4">Features for Your Office</h2>
                        <p className="text-wood/60">Everything you need to manage devotees and rituals smoothly.</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                         <FeatureBullet 
                            title="Easy Printed Receipts" 
                            desc="Print professional receipts instantly for devotees in English or Malayalam." 
                         />
                         <FeatureBullet 
                            title="WhatsApp Receipts" 
                            icon={Smartphone}
                            desc="Send receipts directly to the devotee's phone to save paper and cost." 
                         />
                         <FeatureBullet 
                            title="Devotee History" 
                            desc="Automatically save every pooja a devotee has done at your temple." 
                         />
                         <FeatureBullet 
                            title="Daily Income Sync" 
                            desc="Every booking is automatically added to your daily accounts (Daybook)." 
                         />
                         <FeatureBullet 
                            title="Star-Based Logic" 
                            desc="The software checks the star and date so you never book a pooja incorrectly." 
                         />
                         <FeatureBullet 
                            title="Works Offline" 
                            desc="Your counter keeps working even if the internet is slow or disconnected." 
                         />
                    </div>
                </div>
            </section>
...
const FeatureBullet = ({ title, desc, icon: Icon }) => (
    <div className="p-10 rounded-3xl border border-wood/5 bg-cream/50 hover:bg-white hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
        <div className="h-14 w-14 rounded-2xl bg-white flex items-center justify-center text-primary mb-8 shadow-sm border border-wood/5 group-hover:bg-primary group-hover:text-white transition-all">
            {Icon ? <Icon size={20} /> : <Check size={20} strokeWidth={3} />}
        </div>
